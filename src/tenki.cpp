...
Tenki::Tenki() : json(22528), _isEnable(false) { readJson(); }
...
int Tenki::getBeaufortScale(int i) {
    static const float speed_tbl[] = {0.2,  1.5,  3.3,  5.4,  7.9,  10.7,
                                      13.8, 17.1, 20.7, 24.4, 28.4, 32.6};
    float speed = getWindSpeed(i);
    int scale = 0;
    while (scale < 12 && speed > speed_tbl[scale]) {
        scale++;
    }
    return scale;
}
...
const char* Tenki::getWindDir(int i) {
    static const char* dir[] = {"北", "北東", "東", "南東",
                                "南", "南西", "西", "北西"};
...
int Tenki::getListIdSpecifiedTime(time_t time) {
    ...
    while (listNo <= max && getDate(listNo) < time) {
        listNo++;
    }
    return listNo;
}
